...
export default async function RestaurantCustomers({searchParams}:{searchParams:Promise<{page?:string;q?:string}>}) {
  ...
  return <section className="dash-main"><header><div><small>{t("nav")}</small><h1>{t("title")}</h1><p>{t("subtitle")}</p></div></header><article className="dash-card management-card"><form className="management-toolbar"><div className="dashboard-search"><input name="q" defaultValue={q} placeholder={t("search")}/></div></form>{groups.length?<table><thead><tr><th>{t("customer")}</th><th>{t("firstOrder")}</th><th>{t("lastOrder")}</th><th>{t("orders")}</th><th>{t("spending")}</th><th>{t("average")}</th><th>{t("favoriteProducts")}</th><th>{t("lastAddress")}</th></tr></thead><tbody>{groups.map(group=>{const detail=details.get(group.customerPhone);const products=detail?[...detail.products.entries()].sort((a,b)=>b[1]-a[1]).slice(0,3).map(([name])=>name).join(", "):"—";return <tr key={group.customerPhone}><td><strong>{detail?.name||t("guest")}</strong><small>{group.customerPhone}</small></td><td>{group._min.createdAt?new Intl.DateTimeFormat(locale,{dateStyle:"medium"}).format(group._min.createdAt):"—"}</td><td>{group._max.createdAt?new Intl.DateTimeFormat(locale,{dateStyle:"medium"}).format(group._max.createdAt):"—"}</td><td>{group._count._all}</td><td>{money(Number(group._sum.total||0))}</td><td>{money(Number(group._avg.total||0))}</td><td>{products}</td><td>{detail?.address||"—"}</td></tr>})}</tbody></table>:<p>{t("empty")}</p>}<div className="pagination">{page>1&&<Link href={`?page=${page-1}&q=${encodeURIComponent(q)}`}>{common("previous")}</Link>}<span>{page} / {Math.max(1,Math.ceil(totalGroups/take))}</span>{page*take<totalGroups&&<Link href={`?page=${page+1}&q=${encodeURIComponent(q)}`}>{common("next")}</Link>}</div></article></section>;
}
