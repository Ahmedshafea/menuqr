...
export default async function DashboardSecurityPage({ searchParams }: { searchParams: Promise<{ result?: string }> }) {
  ...
  async function changePassword(form: FormData) {
    ...
    const result = await changeAuthenticatedPassword({ userId: session.user.id, ip: requestIpFromHeaders(await headers()), currentPassword, newPassword: password, confirmPassword });
    redirect(`/dashboard/security?result=${result === "rate_limited" ? "invalid" : result}`);
  }
