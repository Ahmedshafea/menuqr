...
export default async function AnalyticsPage() {
  ...
  return (
    <section className="dash-main">
      ...
      {promotionsAvailable && promotionSummary && <DashboardDisclosure title={promotionText("title")} summary={promotionSummary._count._all}><div className="stats"><article><p>{promotionText("ordersAffected")}</p><strong>{promotionSummary._count._all}</strong></article><article><p>{promotionText("discountCost")}</p><strong>{money(Number(promotionSummary._sum.discountAmount || 0))}</strong></article><article><p>{promotionText("conversion")}</p><strong>{orders.length ? `${Math.round((promotionSummary._count._all / orders.length) * 100)}%` : "0%"}</strong></article></div><div className="record-list">{promotionTop.map((promotion, index) => <RecordDisclosure key={`${promotion.promotionId}-${promotion.promotionName}`} title={`${index + 1}. ${promotion.promotionName}`} meta={promotion._count._all}><p><b>{promotionText("discountCost")}</b><span>{money(Number(promotion._sum.discountAmount || 0))}</span></p></RecordDisclosure>)}</div></DashboardDisclosure>}
