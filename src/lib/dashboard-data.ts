...
export async function getDashboardData(restaurantId: string) {
  ...
  const [restaurant, todayByStatus, monthRevenue, yearRevenue, products, categories, events, recentOrders, topProducts, recentCustomers, workingHours] = await Promise.all([
    prisma.restaurant.findUniqueOrThrow({ where: { id: restaurantId }, select: { name: true, logoUrl: true, slug: true, currency: true, description: true, descriptionAr: true, address: true, whatsapp: true, settings: { select: { setupChecklistDismissed: true } } } }),
    ...
    prisma.product.groupBy({ by: ["availability"], where: { restaurantId }, orderBy: { availability: "asc" }, _count: { _all: true } }),
    ...
    prisma.workingHour.count({ where: { branch: { restaurantId }, isClosed: false, opensAt: { not: null }, closesAt: { not: null } } }),
  ]);
  const status = Object.fromEntries(todayByStatus.map(row => [row.status, row._count?._all ?? 0]));
  const productCounts = Object.fromEntries(products.map(row => [row.availability, row._count?._all ?? 0]));
  ...
  return { restaurant, todayOrders: todayByStatus.reduce((sum,row)=>sum+(row._count?._all??0),0), pendingOrders: status.NEW??0, completedOrders: status.COMPLETED??0, cancelledOrders: status.CANCELLED??0, revenueToday: todayByStatus.filter(row=>row.status==="COMPLETED").reduce((sum,row)=>sum+Number(row._sum?.total??0),0), revenueMonth:Number(monthRevenue._sum.total??0), revenueYear:Number(yearRevenue._sum.total??0), totalProducts, activeProducts:productCounts.AVAILABLE??0, hiddenProducts:productCounts.HIDDEN??0, categories, workingHours, qrScans:eventCounts.QR_SCAN??0, menuViews:eventCounts.MENU_VIEW??0, recentOrders, topProducts, recentCustomers };
}
